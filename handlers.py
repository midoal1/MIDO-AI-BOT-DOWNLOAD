...
from database import register_user, get_user_lang, set_user_lang, increment_downloads, get_stats
...
TEXTS = {
    "ar": {
        "welcome": (
            ...
            "💡 <b>كيفية الاستخدام:</b>\n"
            "فقط قم بإرسال رابط أي فيديو للتنزيل أو اختر اللغة أدناه:"
        ),
        "help": (
    ...
    "en": {
        "welcome": (
            ...
            "💡 <b>How to use:</b>\n"
            "Just send any video link to download or select language below:"
        ),
        "help": (
...
def get_lang_keyboard(current_lang: str):
...
async def start_handler(message: Message):
    ...
    t = TEXTS[ulang]
    welcome_text = t["welcome"].format(user_count=user_count)
    keyboard = get_lang_keyboard(ulang)
...
async def lang_command_handler(message: Message):
    ...
    t = TEXTS[ulang]
    keyboard = get_lang_keyboard(ulang)
    await message.answer(t["lang_prompt"], reply_markup=keyboard, parse_mode="HTML")
...
async def handle_set_language(callback: CallbackQuery):
    ...
    stats = get_stats()
    welcome_text = t["welcome"].format(user_count=stats["user_count"])
    keyboard = get_lang_keyboard(new_lang)
